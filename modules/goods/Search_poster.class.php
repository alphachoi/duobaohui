...
//        $log->w_log($tt);
        if (!empty($mask['maskWords'])) {
			$this->view = array('goods' => array('tInfo' => array(), 'totalNum' => 0));
			return TRUE;
        }
		...
		if (!$searchHelper->dataProcess()) {
			if (empty($responsePosterData)) {
				$responsePosterData = array('goods' => array('tInfo' => array(), 'totalNum' => 0));
			}
			$this->view = $responsePosterData;
			return TRUE;
		}

		$tids = $searchHelper->getTids();
		...
		$responsePosterData = array('goods' => array('tInfo' => $poster, 'totalNum' => $totalNum));
