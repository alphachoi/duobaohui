...
class Poster_group extends \Snake\Libs\Controller {
	...
	public function run() {
		...
		$data = $this->getUserGroupSquares();
		if (empty($data['tInfo'])) {
			$this->view = array('tInfo' => array(), 'totalNum' => 0);
			return FALSE;
		}
		//print_r($data);exit;
		$this->view = $data;
	...
	private function getUserGroupSquares() {
		...
		if ($this->type === 'editor') {
			$groupInfo = $uGHelper->getUserGroupSquaresByRole($this->visitedUserId, array(0, 1), $this->offset, $this->limit, $this->userId);
			if (!empty($groupInfo)) {
				foreach ($groupInfo AS $key => $value) {
					$result['tInfo'][] = $groupInfo[$key]; 
				}
			}
			$userGroupHelper = new GroupUser();
			$editorNum = (int) $userGroupHelper->getUserGroupNumber($this->visitedUserId, array(0, 1));
            $result['totalNum'] = $editorNum;
		}
		elseif ($this->type === 'follow') {
			$groupInfo = $uGHelper->getUserFollowGroupSquares($this->visitedUserId, $this->offset, $this->limit, $this->userId);
			if (empty($groupInfo)) {
				$result['tInfo'] = array();
				$result['totalNum'] = 0;
			}
				...
				$totalNum = RedisUserGroupFollower::getFollowGroupCount($this->visitedUserId);
	...
	private function setType() {
		$type = !empty($this->request->REQUEST['group']) ? $this->request->REQUEST['group'] : '';
