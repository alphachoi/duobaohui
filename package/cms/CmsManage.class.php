...
class CmsManage{
	...
	static public function getCmsData($params, $selectComm) {
		...
		$domainObjectAssembler = new DomainObjectAssembler(CmsManagePersistenceFactory::getFactory('\Snake\Package\Cms\CmsManagePersistenceFactory'));
		$cmsManageCollection = $domainObjectAssembler->mysqlFind($identityObject);

		while ($cmsManageCollection->valid()) {
			$cmsManageObj = $cmsManageCollection->next();
			$cmsData[] = $cmsManageObj->getData();
		}
		return $cmsData;
	}
