...
class MobileTopClient2
{
	...
	public function getAccessToken($code, $redirect_uri) {
		...
		$apiParams = array('grant_type' => 'authorization_code', 'code' => $code, 'redirect_uri' => $redirect_uri, 'view' => 'wap');
		$apiParams['client_id'] = $this->appkey;
		$apiParams['client_secret'] = $this->secretKey;
		try {
			$resp = $this->curl(TAOBAO_TOKEN_URL, $apiParams);
			$accessTokenArr = json_decode($resp, TRUE);
					
			//var_dump($accessTokenArr); die();
