...
class RedisPopularHelper extends \Snake\Libs\Redis\Redis {
	...
	public static function removeTidInRedis($revTids, $keyName) {
		$tidJson = parent::get($keyName);	
		if (empty($tidJson)) {
			return FALSE;
		}
		$tidsObj = json_decode($tidJson);
		$tidsTmp = $tidsObj->tids;
		$tids = array();
		foreach ($tidsTmp as $id) {
			$tids[$id] = $id;	
		}
		foreach ($revTids as $tid ) {
			if (isset($tids[$tid])) {
				unset($tids[$tid]);	
			}
		}
		...
		$tids = array('tids' => $tids);
		$tidsJson = json_encode($tids);
		return	parent::set($keyName, $tidsJson);
	}
