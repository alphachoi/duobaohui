...
Use \Snake\Libs\Thrift\TException;
...
class TMlsSocket extends TTransport {
    ...
    public function read($len) {
        ...
        while ($offset < $len) {
            if (($data = @socket_read($this->socket_, $len - $offset)) === false) {
                ...
                if (11 === $errorcode) { // EAGAIN: timeout
                    //continue;
                } else if (4 === $errorcode) { // Interrupted system call
                    MlscacheLog('ERROR', "act=read, errorcode=4, err={$errormsg}");
                    continue;
                }
    ...
    function write($buf) {
        ...
        while (true) {
            ...
            if ($counter > 10) {
                throw new TTransportException('writeFail act=write_0_bytes' . ' id=' . $this->id_);
            } 
            if ($sent < $length) {
                $buf = substr($buf, $sent);
                $length -= $sent;
